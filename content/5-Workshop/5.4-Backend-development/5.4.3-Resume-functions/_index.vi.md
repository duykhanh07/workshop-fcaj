...
Trong bài này, chúng ta sẽ xây dựng **Resume Function**. Đây không chỉ là nơi lưu trữ nội dung CV mà còn là "trợ lý viết lách" cho người dùng.

### Tính năng chính:
1.  **Lưu trữ (Storage):** Lưu toàn bộ nội dung CV dưới dạng **Markdown** vào DynamoDB. Tại sao là Markdown? Vì nó nhẹ, dễ format và dễ chuyển đổi sang PDF.
2.  **Nâng cấp (Improvement):** Gửi các đoạn mô tả kinh nghiệm thô sơ lên AI (Claude 3) để viết lại chuyên nghiệp hơn.

---

## 1. Model (Entity Layer)

Định nghĩa cấu trúc lưu trữ CV trong DynamoDB.
...
* PK: USER#{userId} (Gắn liền với user).
* SK: RESUME (Mỗi user chỉ có 1 bản nháp CV chính trong ngữ cảnh này).
...
Đây là phần thú vị nhất. Hàm improveWithAI sẽ gọi Amazon Bedrock.
...
Module này cần xử lý 3 luồng chính: Lấy CV, Lưu CV và Gọi AI.
