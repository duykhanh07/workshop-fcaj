...
Trong bài cuối cùng của phần Backend, chúng ta sẽ xây dựng chức năng **Industry Insight** (Thông tin thị trường).

Chức năng này giúp người dùng nhập vào một ngành nghề (ví dụ: "React Developer") và nhận lại các thông tin như:
* Mức lương trung bình.
* Kỹ năng yêu cầu.
* Xu hướng tuyển dụng.

### Chiến lược: Cache-First
Gọi AI (Bedrock) tốn phí cho mỗi token. Thông tin thị trường không thay đổi từng giây. Do đó, chúng ta sẽ áp dụng chiến lược:
1.  Người dùng hỏi về "Java Developer".
2.  Kiểm tra DynamoDB xem đã có thông tin này chưa.
3.  Nếu **CÓ**: Trả về ngay lập tức (Nhanh, Rẻ).
4.  Nếu **KHÔNG**: Gọi Bedrock phân tích -> Lưu vào DB -> Trả về (Chậm hơn chút, nhưng chỉ tốn 1 lần).

---

## 1. Model (Entity Layer)

Định nghĩa cấu trúc dữ liệu để lưu trữ kết quả phân tích thị trường vào DynamoDB.
...
* PK: INSIGHT#{tên_ngành_viết_thường} (Ví dụ: INSIGHT#java-developer).
* SK: METADATA.

---

## 2. Repository (Data Access Layer)
Tương tác với DynamoDB để tìm kiếm và lưu trữ Insight.
...
Nơi xử lý logic kết hợp với BedrockService để thực hiện chức năng thống kê bằng AI
...
Hàm Lambda cuối cùng để xây dựng API.
