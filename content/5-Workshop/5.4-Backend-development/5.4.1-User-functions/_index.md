...
In this lesson, we will build the first module: **User Profile Management (User Profile)**.

We will follow the standard Spring Boot process:
1.  **Model:** Define data structure.
2.  **Repository:** Interact with DynamoDB.
3.  **Service:** Handle business logic.
4.  **Function:** Receive requests from API Gateway.

---

## 1. Model (Entity Layer)

First, we need to map the Java object to the table in DynamoDB.
...
* PK: USER#{userId} (Groups data by User).
* SK: METADATA (Identifies this as profile information).

## 2. Repository (Data Access Layer)
This class is responsible for "talking" to DynamoDB.
...
This is where the business logic is contained.
...
Finally, we write the Lambda Handler to receive HTTP Requests from API Gateway, extract information, and call the Service.
