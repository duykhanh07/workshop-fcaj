...
The `AssessmentFunctions` function acts as a virtual interviewer. It performs two main tasks:
1.  **Generate Quiz:** Generates a "fresh" set of multiple-choice questions each time, based on the user's industry.
2.  **Save & History:** Saves test results so users can track their progress via charts.

---

## 1. Model (Entity Layer)

We need to store the results of each interview session.
...
* PK: USER#{userId}.
* SK: ASSESSMENT#{timestamp} (Use timestamp to sort history chronologically).
...
Combine with BedrockService to implement the generateQuiz function using AI
...
This function handles 3 different endpoints for the interview process.
